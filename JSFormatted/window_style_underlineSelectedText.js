javascript:(function() {
 ...
 var alertWithInstructions_underlineText = 1;

 /* Define global variables.                    */ 
 var currentUnderlineCount = 1; /* count number of times run to get unique ids */
 ...
 var keyCombo_underlineText = "Control+;";        /* key combo to change style */
 ...
 /***** MAIN FUNCTION *****/
 function keypressToBookmarklet_underlineText() {
  /* keyboard combo that enablse text to be style underline */
  let lastUnderlineKeypress = sessionStorage.getItem("bm_last_underline_keypress");
  let currentUnderlineKeyPress = event.key;

  /* check if sessionStorage already contains data with above var names */
  if (lastUnderlineKeypress == null) {
   sessionStorage.setItem("bm_last_underline_keypress", event.key);
   lastUnderlineKeypress = sessionStorage.getItem("bm_last_underline_keypress");
  }
  /* variable to check key combo */
  let checkKeyComboUnderlineText = 
   lastUnderlineKeypress + "+" + currentUnderlineKeyPress;

  /* check if key combo is ctrl + u */
  if (checkKeyComboUnderlineText == keyCombo_underlineText) {   
   currentUnderlineCount += 1;         /* increment count for unique id   */
   getParentElementOfSelection(); /* get parent of selected text     */
   
   /* add a delay to ensure two previous blocks run */ 
   setTimeout(function() { 
    underlineTheSelectedText(); /* wrap the selected text in <u> tag */
   }, 100);
  }
 }
   ...
   if (selection_underlineText.rangeCount > 0) { /* ensure something is selected            */
    ...
    if (selectionParent_underlineText.nodeType === 3) { /* if it is a text node        */
     ...
     if (curPar.hasAttribute("id") == true) {
      let curID = curPar.id;
      if (curID.indexOf("bm_window_style_underlineSelectedText_") > -1) {       
       /* don't include prior unique ids */
       curID = curID.replace(/bm_window_style_underlineSelectedText_[0-9]+/g, "bm_window_style_underlineSelectedText_" + currentUnderlineCount);
       /* set the parents' id to unique id */
       curPar.id = curID;
      } else {
       /* append unique id to the current id value */
       curPar.id += " bm_window_style_underlineSelectedText_" + currentUnderlineCount;
      }
     } else {
      /* set the parents' id to unique id */
      curPar.setAttribute("id", "bm_window_style_underlineSelectedText_" + currentUnderlineCount);
     }       

     /* select by id to get parent element */
     parentIDWithSelectedText_underlineText = 
      document.getElementById("bm_window_style_underlineSelectedText_" + currentUnderlineCount);
    } else {
     let skip;
    }
   }
 ...
 const addKeyDown_underlineText = () => { 
  document.body.addEventListener("keydown", function() {
   keypressToBookmarklet_underlineText();  
  }); 
 }; 
 ...
 *******************************************************************/ 
 ...
 if (alertWithInstructions_underlineText != undefined && 
  ...
  let alertText = "Press '" + keyCombo_underlineText + "' to make selected text underline. \n\n" +
        "For best results: \n\n" +
